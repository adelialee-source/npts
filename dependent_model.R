...
# Draw power curves
summaries <- list(
  power_unif_25 = df_power_unif_25,
  power_unif_50 = df_power_unif_50,
  power_beta_25 = df_power_beta_25,
  power_beta_50 = df_power_beta_50
)

collect_power <- function() {
  grid <- expand.grid(dist = c("unif", "beta"), n = c(25L, 50L),
                      stringsAsFactors = FALSE)

  wide <- do.call(rbind, Map(function(d, nn) {
    nm <- sprintf("power_%s_%d", d, nn)
    if (is.null(summaries[[nm]])) stop("missing job: ", nm)
    cbind(summaries[[nm]], dist = d, n = nn)
  }, grid$dist, grid$n))

  base <- wide[c("gap", "k", "dist", "n")]
  long <- rbind(cbind(base, power = wide$power_95, level = "5% level"),
                cbind(base, power = wide$power_90, level = "10% level"))

  long$level <- factor(long$level, c("5% level", "10% level"))
  long$dist <- factor(long$dist, c("unif", "beta"),
                       c("Z ~ Uniform(0, 1)", "Z ~ Beta(2, 4)"))
  long$n <- factor(long$n, c(25, 50), c("n = 25", "n = 50"))
  long$k <- factor(long$k, c("350", "450"), c("k* = 350", "k* = 450"))
  long
}

power_figure <- function() {
  ggplot(collect_power(), aes(x = gap, y = power, colour = k, linetype = level)) +
    stat_smooth(method = "gam", formula = y ~ s(x, bs = "cs"),
                se = FALSE, linewidth = 0.7) +
    facet_grid(dist ~ n) +
    scale_colour_manual(values = c("k* = 350" = "grey60", "k* = 450" = "black")) +
    scale_linetype_manual(values = c("5% level" = "solid", "10% level" = "dashed")) +
    coord_cartesian(ylim = c(0, 1)) +
    scale_y_continuous(breaks = seq(0, 1, 0.25)) +
    labs(x = expression(delta), y = "Power", colour = NULL, linetype = NULL) +
...
ggsave("fig_setting2.pdf", power_figure(), width = 6.5, height = 5.0)
